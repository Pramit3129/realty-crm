...
export async function sendEmailToLead(
  ...
  try {
    ...
    if (!leadId || !subject || !body) {
      res
        .status(400)
        .json({ message: "Missing required fields: leadId, subject, body" });
      return;
    }

    // Security: scope lead lookup to the authenticated user
    const lead = await Lead.findOne({ _id: leadId, realtorId: userId }).select(
      "email name",
    );
  ...
  try {
    ...
    try {
      ...
      if (!payload || !payload.email_verified) {
        logger.warn("Webhook token payload invalid", {
    ...
    if (!message || !message.data) {
      res
        .status(400)
        .json({ message: "Bad Request: Invalid Pub/Sub message format" });
      return;
    }
    ...
    try {
      ...
      logger.error("Webhook received malformed JSON in Pub/Sub data", {
        raw: dataStr,
      });
    ...
    if (!emailAddress || !historyId) {
      logger.warn("Webhook received message with missing fields", {
        emailAddress,
        historyId,
      });
    ...
    logger.info("Webhook enqueued Cloud Tasks processing job", {
      emailAddress,
      historyId,
    });
    ...
    logger.error("Error in webhook receiver", {
      error: error.message,
      stack: error.stack,
    });
...
export async function processWebhookWorker(
  req: Request,
  res: Response,
): Promise<void> {
  try {
    ...
    if (!emailAddress || !historyId) {
      logger.warn("Worker received task with missing fields", {
        emailAddress,
        historyId,
      });
      res.status(200).send("OK");
