...
export default function Sidebar({
  ...
  const websiteBuilderUrl = process.env.WEBSITE_BUILDER || "https://realtygenie.org";
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex h-full w-56 shrink-0 flex-col bg-sidebar text-sidebar-foreground transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
          ...
          {NAV_ITEMS.map((item) => {
              ...
              return (
                <div key={item.key} className="flex flex-col gap-0.5">
                  <button
                    ...
                    className={`flex items-center gap-3 rounded-lg px-3 py-1.5 text-[13px] font-medium transition-colors ${isActiveParent
                    ...
                    <div className="mt-0.5 flex flex-col gap-0.5 pl-6">
                      {item.subItems.map((subItem) => (
                        <button
                          ...
                          className={`flex items-center gap-3 rounded-lg px-3 py-1.5 text-[13px] font-medium transition-colors ${activeView === subItem.key
                            ? "bg-white/[0.06] text-sidebar-foreground"
                            : "text-muted-foreground hover:bg-white/[0.04] hover:text-sidebar-foreground"
                            }`}
                        >
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 rounded-lg px-3 py-1.5 text-[13px] font-medium transition-colors ${activeView === item.key
                  ? "bg-white/[0.06] text-sidebar-foreground"
                  : "text-muted-foreground hover:bg-white/[0.04] hover:text-sidebar-foreground"
                  }`}
              >
        ...
        <nav className="flex flex-col gap-0.5 px-2 pb-4">
          <button
            ...
            className={`flex items-center gap-3 rounded-lg px-3 py-1.5 text-[13px] font-medium transition-colors ${activeView === "settings"
              ? "bg-white/[0.06] text-sidebar-foreground"
              : "text-muted-foreground hover:bg-white/[0.04] hover:text-sidebar-foreground"
              }`}
          >
